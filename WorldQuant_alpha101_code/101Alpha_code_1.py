...
# 符号函数：返回数组中每个元素的符号（正数返回1，负数返回-1，零返回0）
...
def ts_sum(df, window=10):
    ...
    :param window: 滚动窗口大小。
    :return: 返回过去'window'天的时间序列求和结果的pandas DataFrame。

    示例:
    输入数据: [1, 2, 3, 4, 5]
    window=3时的输出: [NaN, NaN, 6, 9, 12]
    解释: 6=1+2+3, 9=2+3+4, 12=3+4+5
    """
...
def sma(df, window=10):
    ...
    :param df: pandas DataFrame对象。
    :param window: 滚动窗口大小。
    :return: 返回过去'window'天的时间序列移动平均值的pandas DataFrame。

    示例:
    输入数据: [2, 4, 6, 8, 10]
    window=3时的输出: [NaN, NaN, 4.0, 6.0, 8.0]
    解释: 4.0=(2+4+6)/3, 6.0=(4+6+8)/3, 8.0=(6+8+10)/3
    """
...
def stddev(df, window=10):
    ...
    :param df: pandas DataFrame对象。
    :param window: 滚动窗口大小。
    :return: 返回过去'window'天的时间序列标准差的pandas DataFrame。

    标准差计算公式（样本标准差）:
    std = √[Σ(xi - x̄)² / (n-1)]
    其中: xi是每个数据点, x̄是平均值, n是数据点个数
    除以(n-1)是贝塞尔校正，用于无偏估计

    计算步骤示例（以[1,2,3]为例）:
    1. 计算平均值: x̄ = (1+2+3)/3 = 2
    2. 计算差值: (1-2)=-1, (2-2)=0, (3-2)=1
    3. 平方: (-1)²=1, (0)²=0, (1)²=1
    4. 求方差: (1+0+1)/(3-1) = 1
    5. 开平方: √1 = 1.0
    ...
    示例:
    输入数据: [1, 2, 3, 4, 5]
    window=3时的输出: [NaN, NaN, 1.0, 1.0, 1.0]
    解释: std([1,2,3])=1.0, std([2,3,4])=1.0, std([3,4,5])=1.0
    """
...
def correlation(x, y, window=10):
    """
    计算滚动相关系数的包装函数。
    :param x: pandas DataFrame或Series对象。
    :param y: pandas DataFrame或Series对象。
    :param window: 滚动窗口大小。
    :return: 返回过去'window'天x和y的时间序列相关系数的pandas DataFrame。

    相关系数计算公式（Pearson相关系数）:
    ...
    计算步骤示例（以x=[1,2,3], y=[2,4,6]为例）:
    1. 计算平均值: x̄=2, ȳ=4
    2. 计算偏差:
       x偏差: [-1, 0, 1]
       y偏差: [-2, 0, 2]
    3. 偏差乘积之和: (-1)(-2) + (0)(0) + (1)(2) = 4
    4. x偏差平方和: (-1)² + 0² + 1² = 2
    5. y偏差平方和: (-2)² + 0² + 2² = 8
    6. 相关系数: 4 / √(2×8) = 4/4 = 1.0

    示例:
    输入数据x: [1, 2, 3, 4, 5]
    输入数据y: [2, 4, 6, 8, 10]
    window=3时的输出: [NaN, NaN, 1.0, 1.0, 1.0]
    解释: x和y完全正相关（y=2x），所以相关系数为1.0
    """
...
def covariance(x, y, window=10):
    ...
    :param y: pandas DataFrame或Series对象。
    :param window: 滚动窗口大小。
    :return: 返回过去'window'天x和y的时间序列协方差的pandas DataFrame。

    协方差计算公式:
    Cov(x,y) = Σ[(xi - x̄)(yi - ȳ)] / (n-1)
    取值范围: (-∞, +∞)
    含义:
      Cov > 0: x增加时，y倾向于增加（正相关）
      Cov < 0: x增加时，y倾向于减少（负相关）
      Cov ≈ 0: x和y无线性关系

    与相关系数的关系: r = Cov(x,y) / (σx × σy)
    区别: 协方差保留了变化幅度信息，有单位；相关系数标准化后无量纲
    ...
    2. 计算偏差:
       x偏差: [-1, 0, 1]
       y偏差: [-2, 0, 2]
    3. 偏差乘积: (-1)×(-2)=2, (0)×(0)=0, (1)×(2)=2
    4. 求和并除以(n-1): (2+0+2)/(3-1) = 4/2 = 2.0

    示例:
    输入数据x: [1, 2, 3, 4, 5]
    输入数据y: [2, 4, 6, 8, 10]
    window=3时的输出: [NaN, NaN, 2.0, 2.0, 2.0]
    解释: cov([1,2,3], [2,4,6])=2.0，表示x和y正相关且协同变化
    """
...
def product(df, window=10):
    ...
    :param df: pandas DataFrame对象。
    :param window: 滚动窗口大小。
    :return: 返回过去'window'天的时间序列乘积的pandas DataFrame。
...
def ts_min(df, window=10):
    ...
    :param df: pandas DataFrame对象。
    :param window: 滚动窗口大小。
    :return: 返回过去'window'天的时间序列最小值的pandas DataFrame。
...
def ts_max(df, window=10):
    ...
    :param df: pandas DataFrame对象。
    :param window: 滚动窗口大小。
    :return: 返回过去'window'天的时间序列最大值的pandas DataFrame。
...
def delta(df, period=1):
    ...
    :param df: pandas DataFrame对象。
    :param period: 差分周期。
    :return: 返回当前值减去'period'天前的值的pandas DataFrame。
...
def delay(df, period=1):
    ...
    :param df: pandas DataFrame对象。
    :param period: 滞后周期。
    :return: 返回滞后时间序列的pandas DataFrame。
...
def rank(df):
    """
    计算纵向排名(每列内部排名)。
    对DataFrame的每一列分别排名,比较该列不同行之间的大小关系。
    :param df: pandas DataFrame对象。
    :return: 返回沿列排名的pandas DataFrame（百分比排名，取值范围0-1）。

    计算方法:
    1. 对数据从小到大排序，得到常规排名（1, 2, 3, ...）
    2. 百分比排名 = 常规排名 / 总数
    3. 按原始位置返回结果

    取值含义:
        - 0.2 (20%分位): 有20%的数据 ≤ 该值，有80%的数据 ≥ 该值（较低水平）
        - 0.5 (50%分位): 中位数位置
        - 0.8 (80%分位): 有80%的数据 ≤ 该值，有20%的数据 ≥ 该值（较高水平）
        - 1.0 (100%分位): 最大值

    计算示例（数据=[10, 50, 30, 20, 40]）:
    步骤1 - 排序: 10 < 20 < 30 < 40 < 50
    步骤2 - 常规排名: [1, 5, 3, 2, 4]
    步骤3 - 百分比排名: [1/5, 5/5, 3/5, 2/5, 4/5] = [0.2, 1.0, 0.6, 0.4, 0.8]

    相同值的处理（默认使用average方法）:
    规则：n个相同值占据k到k+n-1的位置，取平均排名 = (k+...+k+n-1)/n

    示例1 - 2个相同值:
    输入数据: [10, 30, 30, 20, 40]
    排序: 10(1) < 20(2) < 30(3,4) < 40(5)pioerngjpeiasughjrpzdiofkgmdzpiougje0aZ*URTJNgazerpiunjftb86tg9u8yh9iu-0mji-
    ...
    常规排名: [5, 2.5, 2.5, 2.5, 2.5]
    百分比排名: [1.0, 0.5, 0.5, 0.5, 0.5]

    示例:
    输入数据: [10, 50, 30, 20, 40]
    输出: [0.2, 1.0, 0.6, 0.4, 0.8]
    解释: 10最小(0.2), 50最大(1.0), 30中等(0.6)

    应用场景 - 横截面比较（同一时刻不同股票）:
    股票收益率: [0.02, 0.05, 0.01, -0.01, 0.03]
    ...
    排名: [0.8, 0.8, 0.8, 0.4, 0.2]
    三只涨停股获得相同排名0.8（平均法保证公平性）

    与ts_rank的区别:
    - rank(): 横截面排名（同一时刻比较不同股票）
    - ts_rank(): 时间序列排名（同一股票比较不同时间）
    """
...
def scale(df, k=1):
    ...
    :param df: pandas DataFrame对象。
    :param k: 缩放因子。
    :return: 返回重新缩放的pandas DataFrame，使得sum(abs(df)) = k。

    示例:
    输入数据: [1, 2, 3, 4]
    k=1时的输出: [0.1, 0.2, 0.3, 0.4]
    解释: sum(abs([1,2,3,4]))=10, 缩放后每个值除以10
    """
...
def ts_argmax(df, window=10):
    ...
    :param df: pandas DataFrame对象。
    :param window: 滚动窗口大小。
    :return: 返回ts_max(df, window)在窗口内出现的天数位置（从1开始）。
...
def ts_argmin(df, window=10):
    ...
    :param df: pandas DataFrame对象。
    :param window: 滚动窗口大小。
    :return: 返回ts_min(df, window)在窗口内出现的天数位置（从1开始）。

    示例:
    输入数据: [3, 5, 1, 4, 2]
    window=3时的输出: [NaN, NaN, 3, 3, 1]
    解释: [3,5,1]中1在位置3, [5,1,4]中1在位置3, [1,4,2]中1在位置1
    """
